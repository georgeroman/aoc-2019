...
case System.argv() do
  ["1"] ->
    how_many.(fn adjacent ->
      Enum.all?(adjacent, fn {a, b} -> a <= b end) and
        Enum.any?(adjacent, fn {a, b} -> a == b end)
    end)
    |> IO.puts()
  ...
  ["2"] ->
    how_many.(fn adjacent ->
      Enum.all?(adjacent, fn {a, b} -> a <= b end) and
        Enum.any?(adjacent, fn {a, b} ->
          a == b and Enum.count(adjacent, fn x -> x == {a, b} end) == 1
        end)
    end)
    |> IO.puts()
end
