defmodule Day10 do
  def areCollinear({x1, y1}, {x2, y2}, {x3, y3}) do
    x1 * (y2 - y3) + x2 * (y3 - y1) + x3 * (y1 - y2) == 0
  end

  def detectableFrom(asteroids, {x1, y1}) do
    {order, detectable, _} = 
      Enum.reduce(asteroids, {%{}, MapSet.new(), MapSet.put(MapSet.new(), {x1, y1})}, fn ({x2, y2}, {order, detectable, non_detectable}) ->
        if MapSet.member?(non_detectable, {x2, y2}) do
        ...
        else
          {
            ...
            Enum.reduce(asteroids, MapSet.put(non_detectable, {x2, y2}), fn ({x3, y3}, non_detectable) ->
              if areCollinear({x1, y1}, {x2, y2}, {x3, y3}) and {(x3 - x1) > 0, (y3 - y1) > 0} == {(x2 - x1) > 0, (y2 - y1) > 0} do
...
case System.argv() do
  ["1"] ->
    asteroids
      |> Enum.reduce({0, {-1, -1}}, fn ({x1, y1}, {maxCount, coords}) ->
          asteroids =
           ...
           max(
             {maxCount, coords},
             {Day10.detectableFrom(asteroids, {x1, y1}) |> elem(1) |> MapSet.size, {x1, y1}})
         end)
      |> IO.inspect
  ...
  ["2"] ->
    ...
    Day10.detectableFrom(asteroids, {x1, y1})
      |> elem(0)
